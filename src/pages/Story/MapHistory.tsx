...
import type { SessionHistoryResponse, PointHistoryDto } from "@/utils/types";
...
export default function MapHistory({ history }: Props) {
  ...
  useEffect(() => {
    ...
    const makeMarker = (p: any, label: string) => {
      ...
      if (p.type === "MEMO" && p.text) contentParts.push(`📝 ${escapeHtml(p.text)}`);
      if (p.type === "PHOTO" && p.photoPath) contentParts.push(`📷 ${escapeHtml(p.photoPath)}`);
  ...
  return (
    <div style={{ width: "100%", maxWidth: 400 }}>
      ...
      {/* 간단한 범례 */}
      <div style={{ display: "flex", gap: 10, marginTop: 10, fontSize: 12, color: "#555" }}>
