...
import { createSession, acceptSession, getSession } from "../../api/sessionSmoke";
...
export default function WsSmokeTestPage() {
  ...
  const push = (s: string) => setLog((p) => [`${new Date().toLocaleTimeString()}  ${s}`, ...p]);
  ...
  const connect = (sid: number) => {
    ...
    wsRef.current = openSessionWs(sid, token, {
      ...
      onMessage: (d) => push(`WS <- ${typeof d === "string" ? d : JSON.stringify(d)}`),
    });
  };
  ...
  const onAcceptAndConnect = async () => {
    if (sessionId === "" || !Number(sessionId)) return push("set sessionId first");
    TokenService.setToken(TOKENS[who]);
  ...
  const sendPoint = () => {
    ...
    const payload: WsPayload = { type: "POINT", lat: 37.1, lng: 127.1, ts: Date.now(), text: "smoke" };
    ...
    <div style={{ padding: 16, maxWidth: 960 }}>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 12, flexWrap: "wrap" }}>
        <label style={{ display: "flex", gap: 6, alignItems: "center" }}>
          user:
          <select value={who} onChange={(e) => setWho(e.target.value as UserKey)}>
            <option value="user1">user1 (test)</option>
            <option value="user2">user2 (test2)</option>
          </select>
        </label>

        <button onClick={onCreateAndConnect}>2) Create session + WS connect</button>
        ...
        <button onClick={onAcceptAndConnect}>3) Accept session + WS connect</button>
      ...
      <div style={{ border: "1px solid #ddd", borderRadius: 8, padding: 12, height: 380, overflow: "auto" }}>
        {log.map((l, i) => (
      ...
      <div style={{ marginTop: 10, fontSize: 12, color: "#555" }}>
        WS URL 포맷: <code>wss://waffle-project-dev-server.xyz/ws/session?sessionId=...&token=...</code>
      </div>
    </div>
  );
}